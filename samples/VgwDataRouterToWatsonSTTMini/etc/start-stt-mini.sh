...
do
   ...
   streamtool submitjob -d d1 -i i1 --jobname SpeechProcessor$idx_string -P idOfThisSpeechProcessor=$idx_string -P vgwDataRouterTxUrl=wss://<YOUR_HOST>:<YOUR_PORT>/$idx_string -P numberOfSTTEngines=$total_speech_engines_per_speech_processor_job_string -P numberOfResultProcessors=$number_of_result_processors_string -P includeUtteranceResultReceptionTime=true -P sttUri=<YOUR_STT_URI> -P contentType="audio/mulaw;rate=8000" -P redactionNeeded=true -P writeTranscriptionResultsToFiles=false ../output/com.ibm.streamsx.sttgateway.sample.watsonstt.VgwDataRouterToWatsonSTT.sab -C fusionScheme=legacy
...
streamtool submitjob -d d1  -i i1  --jobname VgwDataRouter -P tlsPortForVgwDataRx=9443 -P tlsPortForVgwDataTx=9444 -P totalNumberOfSpeechProcessorJobs=$total_speech_processor_jobs_string -P numberOfSpeechEnginesPerSpeechProcessorJob=$total_speech_engines_per_speech_processor_job_string -P maxConcurrentCallsAllowed=6 -P vgwSessionLoggingNeeded=true -P ipv6Available=true ../../VgwDataRouterMini/output/com.ibm.streamsx.sttgateway.sample.VgwDataRouter.sab  -C fusionScheme=legacy
# ================
current_time=`date`
end_msg="End time: "
